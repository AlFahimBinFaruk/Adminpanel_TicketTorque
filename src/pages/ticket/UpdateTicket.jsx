...
import { useGetTicketDetailsQuery, useUpdateTicketMutation } from '../../services/ticket_api';
...
export default function UpdateTicket() {
    ...
    const { data: details, isLoading, error } = useGetTicketDetailsQuery(ticket_id);
    const [updateTicket] = useUpdateTicketMutation();
    const { data: categoryData, isLoading: isCategoryLoading } = useGetCategoryListQuery();
    const { data: locationData, isLoading: isLocationLoading } = useGetLocationListQuery();
    const { data: vehicleData, isLoading: isVehicleLoading } = useGetVehicleListQuery();
    ...
    const initialValues = {
        ...
        price: details?.price || '',
        qty: details?.qty || '',
        arrival_date: details?.arrival_date ? details.arrival_date.split('T')[0] : '',
        departure_date: details?.departure_date ? formatDateTimeLocal(details.departure_date) : '',
        boarding_point_name: details?.boarding_point_name || '',
        dropping_point_name: details?.dropping_point_name || '',
        from_location_id: details?.from_location.id || '',
        to_location_id: details?.to_location.id || '',
    };

    const handleSubmit = async (values, { resetForm, setSubmitting }) => {
        try {
            await updateTicket({ id: ticket_id, ...values }).unwrap();
            resetForm();
    ...
    if (isLoading || isCategoryLoading || isLocationLoading || isVehicleLoading) {
        return <>Loading..</>;
    }
    ...
    return (
        <div className="update-ticket w-50 m-auto mt-5">
            <h6 className="text-center mb-4">Update Ticket</h6>
            <Formik
